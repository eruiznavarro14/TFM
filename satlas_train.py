...
from models.satlas_model import get_satlas_model
...
def train_model(model, train_loader, val_loader, optimizer, device, num_epochs, patience, save_path):
    best_val_acc = 0.0
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(train_loader, desc="Entrenando"):
            ...
            outputs, loss = model(images, targets=labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            images, labels = images.to(device), labels.to(device)
            outputs, batch_loss = model(images, targets=labels)
...
def main():
    ...
    print(f"\n🚀 Entrenando modelo SATLAS en {device}...")
    model = get_satlas_model(num_classes=10).to(device)
